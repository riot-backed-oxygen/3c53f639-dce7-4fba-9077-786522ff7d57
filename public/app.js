...
function card(a){return '<article class="card" data-id="'+esc(a.actress_id)+'"><div class="portrait">'+(img(a)?'<img loading="lazy" src="'+esc(img(a))+'" alt="'+esc(a.name)+'">':'')+'<span class="badge">'+esc(a.actress_id)+'</span></div><div class="card-info"><h2>'+esc(a.name)+'</h2><p>'+esc(a.name_kana||'')+(a.birthplace?' · '+esc(a.birthplace):'')+'</p></div></article>'}
...
async function showDetail(id){const r=await fetch('/api/actresses/'+id),j=await r.json(),a=j.data;if(!r.ok)return;$('#detail').innerHTML='<div class="detail-wrap">'+(img(a)?'<img class="detail-photo" src="'+esc(img(a))+'">':'<div class="detail-photo"></div>')+'<div class="detail-copy"><div class="eyebrow">PROFILE / '+esc(a.actress_id)+'</div><h2>'+esc(a.name)+'</h2><div class="kana">'+esc(a.name_kana||'')+'</div><div class="facts">'+[['生日',dateOnly(a.birthday)],['出生地',a.birthplace],['身高',a.height_cm&&a.height_cm+' cm'],['三围',a.bust_cm&&a.bust_cm+' / '+(a.waist_cm||'—')+' / '+(a.hip_cm||'—')],['罩杯',a.cup_size]].map(x=>'<div class="fact"><small>'+x[0]+'</small>'+esc(x[1]||'—')+'</div>').join('')+'</div>'+(a.profile_text?'<p class="bio">'+esc(a.profile_text)+'</p>':'')+'</div></div>';$('#modal').showModal()}
...
async function searchMovies(keyword){document.querySelector('#javbusNotice').classList.add('hidden');document.querySelector('#movieGrid').innerHTML='<p class="loading">正在检索…</p>';const oldDetail=document.querySelector('#movieDetail');if(oldDetail)oldDetail.innerHTML='';try{const r=await fetch('/api/javbus/search?keyword='+encodeURIComponent(keyword));const j=await r.json();if(!r.ok)throw Error(j.error);document.querySelector('#movieGrid').innerHTML=(j.movies||[]).map(m=>'<article class="movie-card" data-movie="'+esc(m.id)+'"><img src="'+esc(proxyImage(m.img||''))+'" alt=""><div><b>'+esc(m.id)+'</b><p>'+esc(m.title)+'</p><small>'+esc(m.date||'')+'</small></div></article>').join('')||'<p class="loading">没有找到作品。</p>';document.querySelectorAll('[data-movie]').forEach(x=>x.onclick=()=>movieDetail(x.dataset.movie))}catch(e){document.querySelector('#movieGrid').innerHTML='';javbusMessage(e.message)}}
async function movieDetail(id){try{const r=await fetch('/api/javbus/movies/'+encodeURIComponent(id));const m=await r.json();if(!r.ok)throw Error(m.error);const stars=(m.stars||[]).map(x=>x.name).join('、');const genres=(m.genres||[]).map(x=>'<span>'+esc(x.name)+'</span>').join('');document.querySelector('#moviePageContent').innerHTML='<section class="movie-detail"><div><img class="movie-cover" src="'+esc(proxyImage(m.img||''))+'"></div><div><div class="eyebrow">MOVIE / '+esc(m.id)+'</div><h2>'+esc(m.title)+'</h2><p class="movie-meta">'+esc(m.date||'')+(m.videoLength?' · '+m.videoLength+' min':'')+(stars?' · '+esc(stars):'')+'</p><div class="genres">'+genres+'</div><button class="magnet-btn" id="magnetBtn">查看磁力信息</button><div id="magnetResult"></div></div></section>'+(m.samples&&m.samples.length?'<div class="samples">'+m.samples.map(s=>'<img src="'+esc(proxyImage(s.thumbnail||s.src))+'" loading="lazy">').join('')+'</div>':'');javbusDialog.close();moviePage.showModal();document.querySelector('#magnetBtn').onclick=()=>loadMagnets(m.id,m.gid,m.uc)}catch(e){javbusMessage(e.message)}}
